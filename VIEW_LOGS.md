...
3. **Try registering a user** and watch both terminals for errors

4. **Look for these log entries in order:**
   - Frontend: `"POST /register request received"`
   - Backend: `"POST /auth/register request received"`
   - Backend: `"Validating password strength"`
   - Backend: `"Checking if user already exists"`
   - Backend: `"Creating new user"` or error message
   - Backend: `"Generating authentication token"` or error message
   - Frontend: `"Registration failed"` or `"User registered successfully"`
